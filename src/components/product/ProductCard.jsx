      ...
      <div className="relative aspect-[3/4] bg-stone-100 overflow-hidden mb-4">
        ...
        {isOutOfStock && (
          <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
            <span className="bg-red-600 text-white px-4 py-2 font-medium uppercase tracking-wider text-sm">
              Out of Stock
            </span>
          </div>
        )}
          ...
          <div className="flex items-center">
            {[...Array(5)].map((_, i) => (
              <svg
                key={i}
                className={`w-4 h-4 ${
                  i < Math.floor(averageRating || 0) ? 'text-yellow-500' : 'text-stone-300'
                }`}
                fill="currentColor"
                viewBox="0 0 20 20"
              >
